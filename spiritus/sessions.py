            ...
            for envelope in self._client.events(ready=self._ready, stop=self._stop):
                ...
                for event in normalizer.feed(envelope):
                    ...
                    if isinstance(event, RunCompleted):
                        ...
                        self._fail(RuntimeError(event.message))
                        return
...
class Session:
    ...
    async def send(self, prompt: str) -> RunHandle:
        handle = RunHandle(self._client, self._store, self._audit, self.id)
        await handle._start()
        try:
            await asyncio.to_thread(
                self._client.prompt_async,
                self.id,
                self._prompt_body(prompt),
            )
        except BaseException as exc:
            handle._fail(exc)
            raise
    ...
    async def run(
        ...
        **extra: Any,
        ...
        if output_schema is not None:
            ...
            return RunResult(message, value)
        return result
